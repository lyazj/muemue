...
for epem_dirname in sorted(glob.glob('epem_??_example_*GeV_*_0/')):
    r = re.search(r'^epem_([a-zA-Z])([a-zA-Z])_example_([0-9.eE+-]+)GeV_pT_([0-9.eE+-]+)GeV_eta_([0-9.eE+-]+)_[0-9]+/$', epem_dirname)
    if not r: continue
    positron_polarization, electron_polarization = r.groups()[:2]
    positron_energy, min_lepton_pt, min_lepton_eta = map(float, r.groups()[2:])
    params.append((positron_polarization, electron_polarization, positron_energy, min_lepton_pt, min_lepton_eta))
    ...
    data['xs'][(positron_polarization, electron_polarization, positron_energy, min_lepton_pt, min_lepton_eta)] = ak.mean(tree['Event.Weight'])
    data['E'][(positron_polarization, electron_polarization, positron_energy, min_lepton_pt, min_lepton_eta)] = E
    data['theta'][(positron_polarization, electron_polarization, positron_energy, min_lepton_pt, min_lepton_eta)] = theta

for positron_energy_expected in sorted(set(param[2] for param in params)):
    for key, data_value in data.items():
        plotted = False
        plt.figure(dpi=300)
        if key == 'xs':
            factor = 1
            for (positron_polarization, electron_polarization, positron_energy, min_lepton_pt, min_lepton_eta), value in sorted(data_value.items()):
                if positron_energy != positron_energy_expected: continue
                plotted = True
                print(key, positron_energy, min_lepton_pt)
                label = f'{positron_polarization}{electron_polarization}, $E_{{e^+}} = {positron_energy}\\ \\mathrm{{GeV}},\\ p_{{\\mathrm{{T, min}}}} = ${min_lepton_pt:.2e}$\\ \\mathrm{{GeV}},\\ \\eta_{{\\mathrm{{min}}}} = ${min_lepton_eta:.2e} ($\\times${factor})'
        ...
        plt.grid()
        plt.tight_layout()
        plt.savefig(f'epem_pp_{positron_energy_expected}GeV_{key}.png')
        plt.close()
