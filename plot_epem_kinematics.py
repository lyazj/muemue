...
for epem_dirname in sorted(glob.glob('epem_example_*GeV_*GeV_0/')):
    r = re.search(r'^epem_example_([0-9.eE+-]+)GeV_pT_([0-9.eE+-]+)GeV_E_([0-9.eE+-]+)GeV_0/$', epem_dirname)
    if not r: continue
...
for e1_energy_expected in sorted(set(param[0] for param in params)):
    for min_lepton_energy_expected in sorted(set(param[2] for param in params)):
        for key, data_value in data.items():
            ...
            plt.grid()
            plt.tight_layout()
            plt.savefig(f'epem_{e1_energy_expected}GeV_{key}_E_{min_lepton_energy_expected:.0e}GeV.png')
            plt.close()

for param in params:
    e1_energy, min_lepton_pt, min_lepton_energy = param
    with open(f'epem_example_{e1_energy}GeV_pT_{min_lepton_pt:.0e}GeV_E_{min_lepton_energy:.0e}GeV.txt', 'w') as file:
